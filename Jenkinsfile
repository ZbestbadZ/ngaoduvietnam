pipeline {
    ...
    stages {
                 ...
                 contentReplace(configs: [
                    fileContentReplaceConfig(
                        configs: [
                            ...
                            fileContentReplaceItemConfig(matchCount: 1, replace: '$APP_DEBUG', search: '#APP_DEBUG#'),
                            ...
                            fileContentReplaceItemConfig(matchCount: 0, replace: '$STORAGE_LINK', search: '#STORAGE_LINK#'),
                            fileContentReplaceItemConfig(matchCount: 0, replace: '$TELESCOPE_APP_NAME', search: '#TELESCOPE_APP_NAME#'),
        ...
        stage('PHPUnit Tests') {
            steps {
                sh "./vendor/bin/phpunit -c phpunit.xml --coverage-html ${buildDir}coverage --coverage-clover ${buildDir}coverage/clover.xml --coverage-crap4j ${buildDir}logs/crap4j.xml --log-junit ${buildDir}logs/junit.xml"
            }
        }
        ...
        stage('Deployment') {
            steps {
                ...
                sh """ssh -tt ${SSH_SERVER} << EOF
                        cd ${SERVER_PATH}
                        sudo chown -R jenkins:www-data ${SERVER_PATH}/storage
                        sudo chmod -R 777 ${SERVER_PATH}/storage
                        php artisan cache:clear
                        php artisan config:clear
                        php artisan migrate --force
                        php artisan storage:link
                        ln -s ${STORAGE_LINK} ${SERVER_PATH}/public/uploads
                        sudo chown -R jenkins:www-data ${SERVER_PATH}/public/uploads
                        sudo chmod -R 777 ${SERVER_PATH}/public/uploads
                        exit
                        EOF"""
